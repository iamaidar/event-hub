...
export default function Filter() {
    ...
    useEffect(() => {
        // Загрузка локаций
        axios.get("http://localhost:3000/events/locations", {
            headers: {
        ...
        axios.get("http://localhost:3000/categories/all", {
            ...
            const categoriesData = response.data.data as { id: number; name: string }[];
    ...
    const handleSubmit = () => {
        ...
        if (fromDate) queryParams.append("fromDate", fromDate.toISOString().split("T")[0]);
        if (toDate) queryParams.append("toDate", toDate.toISOString().split("T")[0]);
    ...
    return (
        <div className="flex flex-wrap items-center gap-4 bg-[#04092C] p-4 rounded-lg justify-between md:flex-row flex-col">
            ...
            <div className="relative">
                ...
                >
                    ...
                    {categories.map((category) => (
                        <option key={category.id} value={category.id.toString()} className="text-black">
                            {category.name}
                        </option>
                    ))}
                </select>
            ...
            {/* Дата "от" */}
            <div className="relative">
                <Calendar className="absolute left-3 top-2.5 text-purple-500" size={20} />
                <DatePicker
                    selected={fromDate}
